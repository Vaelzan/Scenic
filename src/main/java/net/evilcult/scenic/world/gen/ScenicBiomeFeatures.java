...
public class ScenicBiomeFeatures {
    ...
    private static final BlockState SANDSTONE_ROCK_PILE = ScenicBlocks.SANDSTONE_ROCK_PILE.get().getDefaultState();
    ...
    public static final OreFeatureConfig ROCKY_DIRT_CONFIG = new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, ROCKY_DIRT, 20);
    public static final SphereReplaceConfig MOSSY_GRAVEL_CONFIG = new SphereReplaceConfig(MOSSY_GRAVEL, 4, 2, Lists.newArrayList(GRASS_BLOCK, DIRT, GRAVEL));
    public static final SphereReplaceConfig MOSSY_ROCKY_DIRT_CONFIG = new SphereReplaceConfig(MOSSY_ROCKY_DIRT, 3, 1, Lists.newArrayList(GRASS_BLOCK, DIRT));

    public static final RockPileConfig ROCK_PILE_CONFIG = (new RockPileConfig(ROCK_PILE, 8, 1, 60, 20, DimensionType.OVERWORLD));
    public static final RockPileConfig SANDSTONE_ROCK_PILE_CONFIG = (new RockPileConfig(SANDSTONE_ROCK_PILE, 8, 61,75, 2, DimensionType.OVERWORLD));
    ...
    public static final BlockClusterFeatureConfig TREASURE_POT_CONFIG = (new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(TREASURE_POT), new SimpleBlockPlacer())).tries(32).whitelist(ImmutableSet.of(COBBLESTONE.getBlock(), MOSSY_COBBLESTONE.getBlock())).func_227317_b_().build();
    ...
    public static void addBiomeFeatures() {
        for (Biome biome : ForgeRegistries.BIOMES) {
            ...
            biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(ROCKY_DIRT_CONFIG).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(8, 0, 0, 128))));
            ...
            if (biome.getCategory() == Biome.Category.DESERT) {
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, ScenicFeatures.SANDSTONE_ROCK_PILE_FEATURE.get().withConfiguration(SANDSTONE_ROCK_PILE_CONFIG).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(10, 0, 0, 20))));
            }
            ...
            if (biome.getCategory() == Biome.Category.RIVER || biome.getCategory() == Biome.Category.SWAMP) {
                ...
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.DISK.withConfiguration(MOSSY_GRAVEL_CONFIG).withPlacement(Placement.COUNT_TOP_SOLID.configure(new FrequencyConfig(1))));
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.DISK.withConfiguration(MOSSY_ROCKY_DIRT_CONFIG).withPlacement(Placement.COUNT_TOP_SOLID.configure(new FrequencyConfig(1))));
            }
